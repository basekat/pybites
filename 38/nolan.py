...
def get_movies():
    ...
    root = get_tree()
    movies = [child.attrib['title'] for child in root]
    return movies
    


def _get_runtime(movie):
    """Takes as input a movie element and returns the movie runtime as int"""
...
def get_movie_longest_runtime():
    ...
    for child in root:
        ...
        runtime.append(_get_runtime(child.attrib))
    return titles[runtime.index(max(runtime))]
